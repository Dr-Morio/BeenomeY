...
public class ApiaryModBlockRecipeHandler{
	...
	public static boolean hasInventoryChanged(ApiaryModBlockEntity pEntity) {
		...
		if (pEntity.lastInventory.size() != currentInventory.size()) {
			BeenomeY.LOGGER.debug("Apiary: Inventory Size Changed - Before: " + pEntity.lastInventory.size() + " Now: " + currentInventory.size());
		...
		for (int i = 4; i < 11; i++) {
			if (currentInventory.get(i) != pEntity.lastInventory.get(i)) {
				BeenomeY.LOGGER.debug("Apiary: Inventory Slot Changed: " + currentInventory.get(i));
				//pEntity.lastInventory = currentInventory;
				saveInventory(pEntity);
				return true;
			}
	...
	public static void saveInventory(ApiaryModBlockEntity pEntity) {
	...
	public static boolean hasValidRecipe(ApiaryModBlockEntity pEntity) {
		
		int bees = getLoadedBees(pEntity);
		int princess = getLoadedPrincess(pEntity);
		
		if((bees >= 3 && princess == 1) || bees == 6) {
			return true;
		} else {
			return false;
		}
	...
	public static ApiaryMode getMode(ApiaryModBlockEntity pEntity) {
		int bees = getLoadedBees(pEntity);
		int princess = getLoadedPrincess(pEntity);
		
		if(bees >= 3 && princess == 1) {
			return ApiaryMode.BREED;
		} else if (bees == 6) {
			return ApiaryMode.DECIDE;
		} else {
			return ApiaryMode.IDLE; //That should not be return if everything works right.
		}
	}
	
	private static int getLoadedPrincess(ApiaryModBlockEntity pEntity) {
		return (pEntity.itemHandler.getStackInSlot(10).getItem().getClass() == ItemPrincess.class) ? 1 : 0;
	}

	private static int getLoadedBees(ApiaryModBlockEntity pEntity) {
		int bees = 0;
		for (int i = 4; i < 10; i++) {
			ItemStack st = pEntity.itemHandler.getStackInSlot(i);
			if (st.getItem().getClass() == ItemBee.class) {
				bees++;
			}
		}
		return bees;
	...
	public static boolean isWorking(ApiaryModBlockEntity pEntity) {
		return (pEntity.getActiveMode() == ApiaryMode.BREED || pEntity.getActiveMode() == ApiaryMode.DECIDE) ? true : false;
	}

	public static void addProgress(ApiaryModBlockEntity pEntity) {

		if(pEntity.getActiveMode() == ApiaryMode.DECIDE) {
			pEntity.progress++;
		}
		
		if(pEntity.getActiveMode() == ApiaryMode.BREED) {
			//Get Princess for multiplication
			...
			BeenomeY.LOGGER.debug("APIARY - RANDOM TICK: " + referencePositiveTickChance + " / " + randomTickChance);
			
			pEntity.progress++;//Normal Progress
			
			if(randomTickChance >= 0) {
				if (randomTickChance >= referencePositiveTickChance) {
					pEntity.progress++;//Positive Progress by Chance
					BeenomeY.LOGGER.debug("APIARY - RANDOM TICK: TICK");
				}
			} else if( randomTickChance <= 0) {
				if (randomTickChance >= referenceNegativeTickChance) {
					BeenomeY.LOGGER.debug("APIARY - RANDOM TICK: NO TICK");
	...
	public static void startCrafting(ApiaryModBlockEntity pEntity) {
		int emptyBeeSlots = 0;
	    ...
	    // Breed new bee
	    if(pEntity.getActiveMode()  == ApiaryMode.BREED && emptyBeeSlots != 0 && emptyBeeSlots != 6 && pEntity.itemHandler.getStackInSlot(10).getItem() != Items.AIR) {
